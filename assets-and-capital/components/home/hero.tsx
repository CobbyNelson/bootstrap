...
export function Hero() {
  return (
    <section className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
      {/*
       * Right-hand image, faded out as it travels left so it dissolves into
       * the text column rather than ending on a hard edge. Two masks: one
       * horizontal (into the copy), one vertical (into the section edges).
       */}
      <ImageLayer
        src={IMAGERY.heroTower.src}
        opacity={0.5}
        position="right center"
        className="left-auto right-0 w-full sm:w-[70%] lg:w-[58%]"
        mask="linear-gradient(to left, rgba(0,0,0,1) 30%, rgba(0,0,0,0) 95%), linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 20%, rgba(0,0,0,1) 78%, rgba(0,0,0,0) 100%)"
      />
