            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="ml-auto text-[0.7rem] text-ink/40">{c.time}</span>
                </div>
                <p className="truncate text-xs text-ink/45">{c.role}</p>
                <div className="mt-1 flex items-center gap-2">
                  <p className={cn("truncate text-xs", c.unread ? "font-medium text-ink/70" : "text-ink/45")}>{c.last}</p>
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-3 border-b border-ink/[0.06] p-4">
          ...
          <div>
            ...
            <p className="text-xs text-ink/45">{active.online ? "Online" : active.role}</p>
          </div>
          <div className="ml-auto flex items-center gap-1">
            {[Phone, Video, Star, MoreHorizontal].map((I, i) => (
              <button key={i} className="grid h-9 w-9 place-items-center rounded-full text-ink/50 hover:bg-paper-2 hover:text-ink">
                <I className="h-4 w-4" />
              </button>
            ))}
          </div>
          ...
          {active.msgs.map((m, i) => (
            <div key={i} className={cn("flex", m.from === "me" ? "justify-end" : "justify-start")}>
              <div className={cn("max-w-[75%] rounded-2xl px-3.5 py-2.5 text-sm", m.from === "me" ? "bg-brand-600 text-white" : "bg-white text-ink ring-1 ring-ink/[0.06]")}>
                <p>{m.text}</p>
                <span className={cn("mt-1 flex items-center justify-end gap-1 text-[0.65rem]", m.from === "me" ? "text-white/70" : "text-ink/40")}>
                  {m.time}
                  {m.from === "me" && <CheckCheck className={cn("h-3 w-3", m.read ? "text-white" : "text-white/50")} />}
                </span>
              </div>
            </div>
          ))}
        ...
        <div className="border-t border-ink/[0.06] p-3">
          <div className="flex items-center gap-2 rounded-full border border-ink/10 bg-paper-2/50 py-1.5 pl-2 pr-1.5">
            <button className="grid h-9 w-9 place-items-center rounded-full text-ink/50 hover:text-ink"><Paperclip className="h-4 w-4" /></button>
            <input
              value={draft}
